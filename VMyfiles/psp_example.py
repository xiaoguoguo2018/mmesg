...
model = dict(
    ...
    backbone=dict(
        ...
        num_stages=4,  # 主干网络状态(stages)的数目，这些状态产生的特征图作为后续的 head 的输入。使用全部4组卷积
...
dataset_type = 'PotsdamDataset'  # 数据集类型，这将被用来定义数据集。
data_root = '/home1/xiaotao/dataset/potsdam_processing/'  # 数据的根路径。
...
test_pipeline = [
    ...
    dict(
        ...
        img_scale=(256, 256),  # 决定测试时可改变图像的最大规模。用于改变图像大小的流程。
...
data = dict(
    ...
    train=dict(  # 训练数据集配置
        type='PotsdamDataset',  # 数据集的类别, 细节参考自 mmseg/datasets/。
        data_root='/home1/xiaotao/dataset/potsdam_processing/',  # 数据集的根目录。
        img_dir='train',  # 数据集图像的文件夹。
        ann_dir='train_labels',  # 数据集注释的文件夹。
...
runner = dict(
    ...
    max_iters=100) # 全部迭代轮数大小，对于 EpochBasedRunner 使用 `max_epochs` 。
